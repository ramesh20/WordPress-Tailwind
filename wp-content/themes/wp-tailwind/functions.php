...
function wp_tailwind_setup() {
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'wp-tailwind' ),
		)
	);
	...
	add_theme_support(
		'custom-background',
		apply_filters(
			'wp_tailwind_custom_background_args',
			array(
 ...
 */
function wp_tailwind_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'wp_tailwind_content_width', 640 );
}
add_action( 'after_setup_theme', 'wp_tailwind_content_width', 0 );
...
function wp_tailwind_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'wp-tailwind' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'wp-tailwind' ),
